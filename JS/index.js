...
async function getWeather(myPosition) {
    try {
        let response = await fetch(`https://api.weatherapi.com/v1/forecast.json?key=${myApiKey}&q=${myPosition}&days=3`);
        if (!response.ok) {
...
function displayWeatherData(data) {
    try {
        ...
        for (let i = 0; i < dataArray.length; i++) {
            ...
            if (i === 0) {
                windStatus = `
                    <span><img src="images/icon-umberella.png" alt="">${dataArray[i].day.daily_chance_of_rain}%</span>
